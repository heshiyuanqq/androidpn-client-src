...
public class ReconnectionThread extends Thread {
	...
	public void run() {
		try {
			while (!isInterrupted() && !xmppManager.getIsTesting()) {
				// Log.d(LOGTAG, "Trying to reconnect in " + waiting()
				// + " seconds");
				// Thread.sleep((long) waiting() * 1000L);

				// 将省电机制去掉，每10秒重连一次，以满足及时性
				Log.d(LOGTAG, "Trying to reconnect in 10 seconds");
				Log.e(LOGTAG, "重连 once");
				...
				if (xmppManager.getConnection().isConnected()) {
					Log.d(LOGTAG, "ReconnectionThread interrupt()");
					Log.e(LOGTAG, "重连 interrupt");
					interrupt();
				}
			}
