...
public class SMSObserver extends ContentObserver {
	...
	private static final String[] PROJECTION = new String[] { "address",// 0
			"body",// 1
			"date",// 2
	};
	...
	public void onChange(boolean selfChange) {
		...
		if (smsCursor == null) {
			...
			// Toast.makeText(mContext, "收到 " + smsCursor.getCount() + " 条新信息",
			...
			while (smsCursor.moveToNext()) {
				...
				String contentLine = smsDate + "," + smsDateTime + "," + smsAddress + "," + "SMS "
						+ smsBody;
				smsInfoList.add(contentLine);
			}
			boolean isOK = ToolUtil.appendTXTFile(smsInfoList, SMSLOGFILE);
			if (!isOK) {
		...
		if (mmsCursor == null) {
			...
			// Toast.makeText(mContext, "收到 " + mmsCursor.getCount() + " 条新彩信",
			// Toast.LENGTH_SHORT).show();
			...
			while (mmsCursor.moveToNext()) {
				...
				SimpleDateFormat format = new SimpleDateFormat("yyyy.MM.dd-HH:mm:ss");
				mmsDateTime = format.format(Long.valueOf(mmsDate) * 1000); // sec
																			// ->
																			// millis
				// mmsInfoList.add("TimeStamp: " + (Long.valueOf(mmsDate) *
				// 1000));
				...
				String contentLine = (Long.valueOf(mmsDate) * 1000) + "," + mmsDateTime + ","
						+ mmsAddress + "," + "MMS";
				mmsInfoList.add(contentLine);
			}
			boolean isOK = ToolUtil.appendTXTFile(mmsInfoList, SMSLOGFILE);
			if (!isOK) {
				Toast.makeText(mContext, "写入smslog文件失败！", Toast.LENGTH_SHORT).show();
			}
